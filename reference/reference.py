...
class TrainingArguments(transformers.Seq2SeqTrainingArguments):
    ...
    sortby: str = field(
        default='none',
        metadata={"help": "How to sort the layers for training (random/alpha)."}
    )
...
def get_accelerate_model(args, checkpoint_dir):
    ...
    if args.full_finetune:
        ...
        print(f"\n\nTotal Number of layers: {count}")
        count = 0 
        random_layers = random.sample(unique_prefixes, args.num_layers)
    ...
    if 'lora' in args.sortby.lower():
        ...
        # if 'layer' or 'alpha' in args.sortby:
        #     stats_path = os.path.join(f'/rscratch/tpang/kinshuk/RpMKin/qlora/output/{args.sortby}/', "stats")
        #     Path(stats_path).mkdir(parents=True, exist_ok=True)
        #     watcher = ww.WeightWatcher(model=model)
        #     ww_details = watcher.analyze(min_evals=10)
        #     ww_details.to_csv(os.path.join(stats_path, f"ww.csv")) # type: ignore
        #     filtered = ww_details[ # type: ignore
        #         ww_details["longname"].str.contains("embeddings") == False # type: ignore
        #     ]
        #     sortby = "alpha"
