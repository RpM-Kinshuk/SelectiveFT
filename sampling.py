...
def matrix_size_dependent_sampling(matrix, isconv2d, conv_norm, num_row_samples, Q_ratio, step_size=10):
    ...
    # Pre-allocate a tensor to store all submatrices for batch processing
    batch_submatrices = torch.zeros(
        (total_windows, num_row_samples, num_col_samples), 
    ...
    for i in range(0, num_row_windows):
        ...
        for j in range(0, num_col_windows):
            ...
            batch_submatrices[window_idx] = matrix[
                row_start:row_start + num_row_samples, 
                col_start:col_start + num_col_samples
            ]
            window_idx += 1
    
    # Apply conv_norm if needed
    if isconv2d:
        batch_submatrices *= math.sqrt(conv_norm)
    
    # Batch compute SVD values
    all_eigs = torch.square(torch.linalg.svdvals(batch_submatrices))
    
    # Flatten and sort
    all_eigs = all_eigs.flatten()
    return torch.sort(all_eigs, descending=False).values
...
def sample_conv2d(matrix, conv_norm, Q_ratio, num_row_samples, sampling_ops_per_dim):
    ...
    # Pre-calculate the maximum number of eigenvalues we might collect
    max_eigs_count = 0
    for W in Wmats:
        rows, cols = W.shape
        if rows == cols:
            max_eigs_count += min(rows, cols)
        else:
            division = int(rows // cols)
            max_eigs_count += division * min(cols, cols)
    
    # Pre-allocate tensor for all eigenvalues
    all_eigs = torch.zeros(max_eigs_count, device=Wmats[0].device, dtype=Wmats[0].dtype)
    ...
    for W in Wmats:
        ...
        if rows == cols:
            ...
            eigs = torch.square(torch.linalg.svdvals(submatrix))
            all_eigs[eig_idx:eig_idx+len(eigs)] = eigs
            eig_idx += len(eigs)
        else:
            if Q_ratio > 1:
            ...
            # Batch processing for multiple submatrices
            batch_submatrices = torch.zeros(
                (division, num_row_samples, num_col_samples), 
                device=W.device, 
                dtype=W.dtype
            )
            
            for i in range(division):
                batch_submatrices[i] = W[i*num_row_samples:(i+1)*num_row_samples, :num_col_samples]
            
            batch_submatrices *= math.sqrt(conv_norm)
            batch_eigs = torch.square(torch.linalg.svdvals(batch_submatrices)).flatten()
            
            all_eigs[eig_idx:eig_idx+len(batch_eigs)] = batch_eigs
            eig_idx += len(batch_eigs)
    
    # Only return the valid part of the pre-allocated tensor
    all_eigs = all_eigs[:eig_idx]
    return torch.sort(all_eigs, descending=False).values